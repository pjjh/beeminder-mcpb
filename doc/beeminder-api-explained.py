...
@mcp.tool(
    description="Returns all goals for the current user that is best presented in a table. Set include_full_data=True to get complete goal data including datapoints (warning: can be very large)."
)
def list_goals(include_full_data: bool = False) -> str:
    """List all goals for the current user.

    Args:
        include_full_data (bool): Whether to include full goal data including datapoints.
                                 Default is False to return only essential goal information.

    Returns:
        str: A JSON string representation of goal data.
    """
    try:
        ...
        if include_full_data:
            ...
            # Return only essential information to keep response size manageable
            essential_data = []
            for goal in goals:
                goal_dict = goal.model_dump()
                # Extract only essential fields, excluding large data like datapoints
                essential_fields = {
                    "slug", "title", "goal_type", "rate", "runits", "goalval", "goaldate",
                    "pledge", "safebuf", "losedate", "curval", "won", "lost", "frozen",
                    "limsum", "numpts", "urgencykey", "graph_url", "fineprint", "yaxis"
                }
                essential_goal = {key: goal_dict.get(key) for key in essential_fields if key in goal_dict}
                essential_data.append(essential_goal)
...
def main():
    """Main entry point for the MCP Beeminder server."""
    try:
